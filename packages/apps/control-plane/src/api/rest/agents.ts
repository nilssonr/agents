...
export function registerAgentRoutes(
    app: FastifyInstance,
    deps: { agentService: AgentService; jobService: JobService },
): void {
    app.post(
        '/agents',
        {
        ...
        async (request, reply) => {
            ...
            try {
                ...
                if (err instanceof Error && err.name === 'AgentNotFoundError') {
                    return await reply.status(404).send(createProblemDetail(404, 'Not Found', err.message));
                }
                if (err instanceof Error && err.name === 'AgentPausedError') {
                    return await reply.status(409).send(createProblemDetail(409, 'Conflict', err.message));
                }
                throw err;
            }
        },
